...
public class SalarioMinimoController {
	...
	@Autowired SalarioMinimoService salarioMinimoService;
	...
	public String save(
			@Valid @ModelAttribute("salarioMinimo") SalarioMinimo salarioMinimo,
		...
		try {
			...
			e.printStackTrace();
			redirectAttributes.addFlashAttribute("erro", "Problemas ao salvar salário");
		}
	...
	public String save(Model model, @PathVariable("salario_id") Integer id, RedirectAttributes redirectAttributes) {
		try {
			...
			e.printStackTrace();
			redirectAttributes.addFlashAttribute("erro", "Problema ao editar salário!");
		}
